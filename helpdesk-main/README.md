...
- Receive support emails and create tickets via CloudMailin inbound webhook
...
- **Email**: CloudMailin (inbound), Nodemailer + SMTP (outbound)
...
1. **Build the Docker image**
   ...
   ```bash
docker build -t resolve-now-ai .
   ```

2. **Run locally with Docker**

   ```bash
docker run -p 3000:3000 --env-file server/.env -e NODE_ENV=production resolve-now-ai
   ```
