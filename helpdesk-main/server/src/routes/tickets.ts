...
import { createTicketSchema, ticketListQuerySchema, updateTicketSchema } from "core/schemas/tickets.ts";
...
router.get("/stats", requireAuth, async (_req, res) => {
  ...
  res.json({
    ...
    resolvedTickets: statusCounts.find((c) => c.status === "resolved")?._count._all ?? 0,
    closedTickets: statusCounts.find((c) => c.status === "closed")?._count._all ?? 0,
...
router.patch("/:id", requireAuth, async (req, res) => {
  ...
  if ("assignedToId" in data && data.assignedToId !== ticket.assignedToId) {
    await logAudit("TICKET_REASSIGN", req.user.id, `Reassigned ticket ${id} from ${ticket.assignedToId} to ${data.assignedToId}`);
  }
  if ("status" in data && data.status !== ticket.status) {
    await logAudit("TICKET_STATUS_UPDATE", req.user.id, `Updated ticket ${id} status from ${ticket.status} to ${data.status}`);
  }
  if ("category" in data && data.category !== ticket.category) {
    await logAudit("TICKET_CATEGORY_UPDATE", req.user.id, `Updated ticket ${id} category from ${ticket.category} to ${data.category}`);
  }
