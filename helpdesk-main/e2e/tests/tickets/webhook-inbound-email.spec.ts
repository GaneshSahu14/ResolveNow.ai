...
test.describe("Webhook: Inbound Email", () => {
  test.describe("CloudMailin: Multipart Normalized", () => {
    test("should create a ticket from CloudMailin payload", async ({ request }) => {
      const sender = `cloudmailin${Date.now()}@example.com`;

      const payload = {
        envelope: { from: sender },
        headers: {
          from: `Cloud User <${sender}>`,
          subject: `Login issue ${Date.now()}`,
        },
        plain: "Cannot access my account",
        html: "<p>Cannot access my account</p>",
      };
      ...
      const response = await request.post(
        `${API_BASE_URL}/api/webhooks/inbound-email`,
        {
          ...
          // Send as JSON. The server route uses multer(any), so Express will
          // parse multipart but CloudMailin may also send JSON. We normalize
          // based on req.body.
          data: payload,
        }
      );
      ...
      expect(body.ticket.senderEmail).toBe(sender);
      expect(body.ticket.body).toContain("Cannot access my account");
      expect(body.ticket.bodyHtml).toBe("<p>Cannot access my account</p>");
    });

    test("should thread CloudMailin follow-up with Re: prefix", async ({ request }) => {
      const timestamp = Date.now();
      const sender = `thread${timestamp}@example.com`;
      const baseSubject = `Login issue ${timestamp}`;
      ...
      const replyPayload = {
        ...
        reply_plain: "Still having trouble",
        html: "<p>Still having trouble</p>",
      };
